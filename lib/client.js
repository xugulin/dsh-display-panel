  ...
  factory: (require) => {
    ...
    const TEXT = {
      zh: {
        ...
        /* ——— 设置卡片（设置 → 插件 → 显示器面板） ——— */
        setTitle: '显示器面板',
        setIntro: '这三项在**显示器服务**里生效（服务是独立进程），保存后宿主会自动重启服务让它们生效。'
          + '留空 = 用默认值或环境变量（DSH_VIEW_SIZE / DSH_VIEW_IDLE_MINUTES / DSH_VIEW_INPUT）。',
        setSize: '分辨率',
        setSizeHint: '每个会话的显示尺寸，例如 1600x1000（宽x高）。改完对**新会话**生效，已开的会话保留原尺寸。',
        setIdle: '空闲回收（分钟）',
        setIdleHint: '多久没有请求就回收会话与虚拟显示，0 = 不回收。默认 30。',
        setInput: '允许注入真实键鼠（仅 Windows / macOS）',
        setInputHint: 'Windows / macOS 抓的是**本机真实桌面**：打开后，面板里的点击与按键会真的落在你自己的桌面上。'
          + 'Linux 上每会话是独立虚拟显示，注入本来就是开的，这一项对它没有影响。',
        setInputWarn: '⚠ 这是真实键鼠：面板里的一次误点就是你桌面上的一次真点击。确认你清楚这一点再打开。',
        setSave: '保存',
        setSaving: '保存中…',
        setSaved: '已保存',
        setReset: '恢复默认',
        setInvalidSize: '尺寸要写成 宽x高（例如 1600x1000）',
        setInvalidIdle: '空闲回收要填 0 或正整数（分钟）',
        setUnavailable: '宿主没有提供这一项的设置面（行 id 与插件不匹配，或 dsh 版本与插件半边对不上）。'
          + '请改用环境变量配置：DSH_VIEW_SIZE / DSH_VIEW_IDLE_MINUTES / DSH_VIEW_INPUT。',
        setLoading: '正在读取设置…',
        setReadOnly: '这个页面只能读设置（当前不是本机会话，或宿主只允许内存态）。请在本机的 DSH 页面里改。',
        setRejected: '宿主拒绝了这次修改（值没有生效）。请检查取值范围，或看 DSH 日志。',
        /* ——— 注入确认（改进点 4） ——— */
        injectEnable: '打开注入',
        injectConfirmTitle: '确认要在真实桌面上注入吗？',
        injectConfirmBody: '这个后端抓的是**本机真实桌面**，不是虚拟显示。打开注入后：',
        injectConfirmBullet1: '面板里的点击、拖动、滚轮会真的作用在你自己的鼠标上；',
        injectConfirmBullet2: '面板里敲的键会真的送到当前聚焦的那个窗口（可能正在输入密码）；',
        injectConfirmBullet3: '所有会话共用这一块屏 —— 别的会话也可能看到、也可能点到。',
        injectConfirmSafe: '只看不点，就一直保持关闭。随时可以再关掉（需要重启服务）。',
        injectConfirmOk: '我明白，打开注入',
        injectConfirmCancel: '取消',
        injectEnabling: '正在打开注入…',
        injectEnableFailed: '打开注入失败，请看服务日志',
        injectEnvHint: '这个宿主版本不支持从面板直接打开（宿主半边太旧，或 dsh 版本较老）。'
          + '办法：在「设置 → 插件 → 显示器面板」里打开这一项；'
          + '或杀掉显示器服务、在跑 DSH 的环境里设 DSH_VIEW_INPUT=1 再重启 DSH。',
      },
      en: {
        title: 'Display',
        ...
        dragging: 'Panning…',
        /* ——— settings card (Settings → Plugins → Display panel) ——— */
        setTitle: 'Display panel',
        setIntro: 'These three take effect in the **display service** (a separate process); the host restarts it '
          + 'after a save. Empty = the built-in default, or the environment variable '
          + '(DSH_VIEW_SIZE / DSH_VIEW_IDLE_MINUTES / DSH_VIEW_INPUT).',
        setSize: 'Resolution',
        setSizeHint: 'Per-session display size, e.g. 1600x1000. Applies to **new** sessions; an open session keeps its size.',
        setIdle: 'Idle reclaim (minutes)',
        setIdleHint: 'Reclaim a session and its virtual display after this long with no requests; 0 = never. Default 30.',
        setInput: 'Allow real keyboard/mouse injection (Windows / macOS only)',
        setInputHint: 'On Windows / macOS the backend captures the **real desktop**: once enabled, clicks and keys '
          + 'from this panel land on your own desktop. On Linux every session is an isolated virtual display where '
          + 'injection is already on, so this switch does not affect it.',
        setInputWarn: '⚠ Real input: a stray click in the panel is a real click on your desktop. Only enable this if you are sure.',
        setSave: 'Save',
        setSaving: 'Saving…',
        setSaved: 'Saved',
        setReset: 'Restore defaults',
        setInvalidSize: 'Use WIDTHxHEIGHT, e.g. 1600x1000',
        setInvalidIdle: 'Idle reclaim must be 0 or a positive whole number of minutes',
        setUnavailable: 'This host serves no settings surface for this entry (row id mismatch, or a dsh version the plugin half cannot pair with). '
          + 'Configure with environment variables instead: DSH_VIEW_SIZE / DSH_VIEW_IDLE_MINUTES / DSH_VIEW_INPUT.',
        setLoading: 'Reading settings…',
        setReadOnly: 'Settings are read-only on this page (not a loopback session, or the host keeps them in memory). Change them from the local DSH page.',
        setRejected: 'The host refused this change, so nothing took effect. Check the range, or look at the dsh log.',
        /* ——— injection confirmation (host-half feature) ——— */
        injectEnable: 'Enable injection',
        injectConfirmTitle: 'Inject into the real desktop?',
        injectConfirmBody: 'This backend captures the **real desktop**, not a virtual display. Once enabled:',
        injectConfirmBullet1: 'clicks, drags and wheel events from the panel move your own mouse;',
        injectConfirmBullet2: 'keys typed here go to whatever window has focus (possibly one asking for a password);',
        injectConfirmBullet3: 'every session shares this one screen, so other sessions may see and click it too.',
        injectConfirmSafe: 'Watch-only stays the default if you never enable it, and you can turn it off again (a service restart applies it).',
        injectConfirmOk: 'I understand — enable injection',
        injectConfirmCancel: 'Cancel',
        injectEnabling: 'Enabling injection…',
        injectEnableFailed: 'Could not enable injection; see the service log',
        injectEnvHint: 'This host half is too old (or runs an older dsh) to switch it on from here. '
          + 'Either enable it under Settings → Plugins → Display panel, or stop the display service, '
          + 'set DSH_VIEW_INPUT=1 where dsh runs, and restart dsh.',
      },
    }
    ...
    function Display(props) {
      ...
      /** 注入确认对话框：`真实桌面 + 注入关着` 时点「打开注入」才会置真。 */
      const [injectAsk, setInjectAsk] = useState(false)
       ...
       * 当前是否允许把事件发给远端。
       *
       * 真实桌面 + 注入关闭时是 `false`：那时服务端不会注入（只读观看），
       * 但事件**还是会进队列、还是会打 HTTP**，失败 5 次之后状态条上冒出
       * "输入发送失败" —— 用户看到的是"坏了"，其实只是"没开"。
       * 所以在这里就拦住，并让点击去弹"要不要打开注入"，把原因说清楚。
       * 用 ref 而不是 useState：引擎的监听器是在 effect 里挂的闭包，
       * 读 state 会读到挂载那一刻的旧值。
       */
      ...
      function push(payload) {
        if (!inputAllowedRef.current) {
          // 只读观看：不发事件，但把"为什么点不动"说出来（点一次弹一次确认框）。
          setInjectAsk(true)
          return
        }
       ...
       * 确认之后才真的打开注入。
       *
       * 走宿主新增的 `/config` 路由（由宿主写进 DSH 的行配置，因此**持久**、
       * 且与设置卡片是同一份值），然后重启服务让新环境变量生效。
       * 老版宿主没有这条路由：如实告诉用户当前版本不支持，并给出替代路径 ——
       * 不假装成功，也不把"要不要交出真实键鼠"这件事悄悄做掉。
       *
       * @returns {Promise<boolean>} 是否真的写成功。
       */
      const confirmEnableInput = async () => {
        if (injectBusy) return false
        setInjectBusy(true)
        setInjectError('')
        try {
          const res = await fetch(API + '/config?session=' + encodeURIComponent(sidRef.current || ''), {
            method: 'POST', credentials: 'same-origin',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ inputEnabled: true, restart: true }),
          })
          if (!res.ok) {
            // 404 = 老版宿主没有这条路由；501 = 有路由但没有可用的设置面。
            // 两者都要给"怎么办"，而不是干巴巴一个 HTTP 码（宿主返回的 hint 也带上）。
            let hint = ''
            try {
              const answer = await res.json()
              if (answer && typeof answer.hint === 'string') hint = answer.hint
            } catch (error) { /* 不是 JSON 就算了 */ }
            const base = (res.status === 404 || res.status === 501) ? t.injectEnvHint : t.errHttp(res.status)
            setInjectError(hint ? base + ' —— ' + hint : base)
            return false
          }
          setInjectAsk(false)
          // 服务重启完才会真正生效；宿主这边配置已经落了，轮询会自动刷新徽章。
          return true
        } catch (error) {
          setInjectError(t.injectEnableFailed)
          return false
        } finally {
          setInjectBusy(false)
        }
      ...
      // 引擎的监听器读的是这个 ref（见 inputAllowedRef 的说明）。在渲染里写 ref
      // 是安全的（不是 state，不触发重渲染），而且比 useEffect 更早 —— 事件监听器
      // 在 effect 里挂，挂之前就已经是新值了。
      //
      // ⚠️ **只在有确定信息时才改它**。`inputEnabled` 的兜底是 `true`，所以在
      // `/info` 还没回来、也没有 snapshot 的**第一瞬间**它是 `false` —— 若直接
      // 赋过去，Linux 上用户第一次点画面就会被弹一个"要不要打开注入"的框（其实
      // 注入本来就开着）。未知状态下沿用上一个已知值：真发不出去时服务端会回错，
      // 那条路才是"确实禁止"的证据。
      const inputKnown = !!(snapshot && typeof snapshot.input === 'boolean')
        || !!(info && info.input && typeof info.input.enabled === 'boolean')
      if (inputKnown) inputAllowedRef.current = inputEnabled
      ...
      const bar = h('div', { className: 'ddp-bar' },
        ...
        // 打开注入：**只在真实桌面上出现**（Linux 每会话是独立虚拟显示，注入本来就开着）。
        // 这不是普通开关而是一个不可轻易回退的动作（真实桌面共享、可能正在输密码），
        // 所以点击只负责弹确认框，真正的写入在确认之后。
        (realDesktop && !inputEnabled)
          ? h('button', {
            type: 'button', className: 'ddp-tbtn', 'data-act': 'inject-enable',
            title: t.injectConfirmTitle,
            onClick: () => { setInjectAsk(true); setInjectError('') },
          }, t.injectEnable)
          : null,
        displayStarted
          ? h('button', {
      ...
      h('div', { className: 'ddp-body' },
        ...
        // 注入确认：必须由人显式点过才生效。放在 body 内层（覆盖画面，不动状态条），
        // 且**不**提供"记住我的选择"—— 真实键鼠这件事每次都值得点一下。
        injectAsk
          ? h('div', { className: 'ddp-modal-mask', role: 'dialog', 'aria-modal': 'true' },
            h('div', { className: 'ddp-modal' },
              h('div', { className: 'ddp-modal-title' }, t.injectConfirmTitle),
              h('div', { className: 'ddp-modal-body' }, t.injectConfirmBody),
              h('ul', { className: 'ddp-modal-list' },
                h('li', null, t.injectConfirmBullet1),
                h('li', null, t.injectConfirmBullet2),
                h('li', null, t.injectConfirmBullet3),
              ),
              h('div', { className: 'ddp-modal-safe' }, t.injectConfirmSafe),
              injectError ? h('div', { className: 'ddp-set-err' }, injectError) : null,
              h('div', { className: 'ddp-modal-actions' },
                h('button', {
                  type: 'button', className: 'ddp-btn',
                  disabled: injectBusy,
                  onClick: () => { setInjectAsk(false); setInjectError('') },
                }, t.injectConfirmCancel),
                h('button', {
                  type: 'button', className: 'ddp-btn ddp-btn-danger',
                  'data-act': 'inject-confirm',
                  disabled: injectBusy,
                  onClick: () => { void confirmEnableInput() },
                }, injectBusy ? t.injectEnabling : t.injectConfirmOk),
              ),
            ),
          )
          : null,
    ...
    /**
     * 设置卡片自己的样式表（**单独一张**，不跟面板的大样式表混）。
     *
     * 为什么分开：设置卡片会渲染在 DSH 的**设置页**上，而那张大样式表里有
     * `.ddp-root` 的变量定义与面板专用规则 —— 把整份注进设置页既浪费又可能
     * 影响别的插件。这张只含卡片与对话框用到的类。
     */
    const SETTINGS_CSS = `
      /* ——— 设置卡片（注册进 DSH 设置页） ———
         ⚠️ 卡片渲染在 **DSH 的设置页**，不在面板的 .ddp-root 里，所以那份
         --ddp-* 变量定义在这里**不存在**。第一版直接写 var(--ddp-text)，结果
         color 解析成无效值被整条丢掉（只能靠继承），深浅主题下都不受控。
         这里把需要的那几个变量就地再定义一份（取值与 .ddp-root 那份一致）。
         （CSS 注释里别写反引号：这整段是 JS 模板字符串。） */
      .ddp-set {
        --ddp-border: var(--dsw-alias-border-l2, rgba(128,128,128,.28));
        --ddp-border-strong: var(--dsw-alias-border-l3, rgba(128,128,128,.45));
        --ddp-surface: var(--dsw-alias-bg-skeleton, rgba(128,128,128,.10));
        ...
        color: var(--ddp-text, inherit);
      }
      /* 卡片与对话框里的按钮用的是 .ddp-btn 系列，而那套样式定义在**面板**的
         大样式表里（只随面板加载）。卡片先于面板出现时按钮就是裸的原生样式，
         所以这里补一份（只作用于卡片/对话框的子树，不污染别人）。 */
      .ddp-set .ddp-btn, .ddp-modal .ddp-btn {
        margin-top: 2px; padding: 7px 16px; font: inherit; font-size: 12.5px; border-radius: 8px;
        cursor: pointer; color: var(--ddp-text, inherit);
        border: 1px solid var(--ddp-border-strong);
        background: var(--ddp-surface);
      }
      .ddp-set .ddp-btn:hover:not(:disabled), .ddp-modal .ddp-btn:hover:not(:disabled) {
        border-color: var(--ddp-accent); background: var(--ddp-surface-hover);
      }
      .ddp-set .ddp-btn:disabled, .ddp-modal .ddp-btn:disabled { opacity: .55; cursor: default; }
      .ddp-set .ddp-btn:focus-visible, .ddp-modal .ddp-btn:focus-visible {
        outline: 2px solid var(--ddp-accent); outline-offset: 1px;
      }
      .ddp-set .ddp-btn-primary, .ddp-modal .ddp-btn-primary { border-color: var(--ddp-accent); }
      /* 对话框渲染在面板里（不是设置页），面板那份样式已经覆盖 .ddp-btn；
         这里只保证 .ddp-modal-mask / .ddp-modal 用到的变量在两种宿主下都有值。 */
      .ddp-modal-mask { --ddp-card: var(--dsw-alias-bg-overlay, rgba(24,26,32,.9)); }
      .ddp-modal {
        --ddp-border-strong: var(--dsw-alias-border-l3, rgba(128,128,128,.45));
        --ddp-text: var(--dsw-alias-label-primary, inherit);
        --ddp-warn: var(--dsw-alias-state-warn-primary, #f59e0b);
      ...
      .ddp-set-head { display: flex; flex-direction: column; gap: 4px; }
      .ddp-set-title { font-size: 14px; font-weight: 600; }
      .ddp-set-intro { font-size: 12px; line-height: 1.7; opacity: .72; }
      .ddp-set-field { display: flex; flex-direction: column; gap: 5px; }
      .ddp-set-label { font-size: 12.5px; font-weight: 500; }
      /* 开关行：label 与 input 同一行，点文字也能切（用 <label> 包住） */
      .ddp-set-toggle {
        display: flex; align-items: flex-start; gap: 8px; cursor: pointer;
        font-size: 12.5px; font-weight: 500;
      }
      .ddp-set-toggle input { margin: 2px 0 0; flex: 0 0 auto; }
      .ddp-set-hint { font-size: 11.5px; line-height: 1.7; opacity: .62; }
      .ddp-set-warn {
        font-size: 11.5px; line-height: 1.7; padding: 7px 9px; border-radius: 7px;
        color: var(--ddp-warn);
        border: 1px solid color-mix(in srgb, var(--ddp-warn) 45%, transparent);
        background: color-mix(in srgb, var(--ddp-warn) 12%, transparent);
      }
      .ddp-set-input {
        font: inherit; font-size: 12.5px; padding: 6px 9px; border-radius: 7px; width: 100%;
        max-width: 260px; box-sizing: border-box;
        color: var(--dsw-alias-label-primary, var(--ddp-text));
        background: var(--dsw-alias-bg-layer-1, var(--ddp-surface));
        border: 1px solid var(--ddp-border-strong);
      }
      .ddp-set-input:focus-visible { outline: 2px solid var(--ddp-accent); outline-offset: 1px; }
      .ddp-set-input[aria-invalid="true"] { border-color: var(--ddp-bad); }
      .ddp-set-actions { display: flex; align-items: center; gap: 9px; flex-wrap: wrap; }
      .ddp-set-note { font-size: 11.5px; opacity: .72; }
      .ddp-set-err { font-size: 11.5px; color: var(--ddp-bad); }
      /* ——— 注入确认对话框（覆盖在画面之上，两种主题下都要醒目） ——— */
      .ddp-modal-mask {
      ...
      .ddp-modal {
        width: min(460px, 100%); padding: 16px 18px; border-radius: 12px;
      ...
      .ddp-modal-title { font-size: 14px; font-weight: 600; }
      .ddp-modal-body { font-size: 12.5px; line-height: 1.75; opacity: .86; }
      .ddp-modal-list { margin: 0; padding-left: 20px; font-size: 12.5px; line-height: 1.75; }
      .ddp-modal-list li { margin: 3px 0; }
      .ddp-modal-safe { font-size: 11.5px; line-height: 1.7; opacity: .62; }
      .ddp-modal-actions { display: flex; gap: 9px; justify-content: flex-end; flex-wrap: wrap; margin-top: 2px; }
      .ddp-modal-actions .ddp-btn { margin-top: 0; }
      .ddp-btn-danger {
        border-color: var(--ddp-warn); color: var(--ddp-warn);
        background: color-mix(in srgb, var(--ddp-warn) 12%, transparent);
      }
      .ddp-btn-danger:hover:not(:disabled) { background: color-mix(in srgb, var(--ddp-warn) 20%, transparent); }
      .ddp-pill {
    ...
    /** 注入设置卡片样式（只一次，按 id 去重）。 */
    function ensureSettingsStyle() {
      try {
        if (document.getElementById('ddp-settings-style')) return
        const el = document.createElement('style')
        el.id = 'ddp-settings-style'
     ...
     * 设置卡片：分辨率 / 空闲回收 / 输入注入
     *
     * 三代的设置面**不相同**（已按 DSH 源码逐 tag 核对），这里做一层适配：
     *
     *   ① dsh ≥ 0.1.7-alpha.1：宿主导出 `Config`（行 Config），客户端从
     *      `ctx.configForms.get(<行 id>)` 拿一个 ConfigForm；
     *   ② dsh 0.1.5-alpha.1 ~ 0.1.6-alpha.2：宿主 `settings.register(ns, schema)`，
     *      客户端 `ctx.settingsScope.bind({ namespace })`；
     *   ③ 卡片槽位另有一条分界：0.1.6-alpha.1 及更早是 `settings.plugin.item`
     *      （key = namespace），0.1.6-alpha.2 起换成 `plugins.bundle.config`
     *      （key = 包名）。同一个 version 里两者**不会同时存在**，所以两个
     *      `slots.inject` 各自等自己的声明，任一版本上恰好只有一个生效。
     *
     * ⚠️ **绝不能**把 `configForms` / `settingsScope` 写进静态的
     * `inject: ['slots']`：服务缺席时整份 apply 会静默 PENDING（面板一起没了）。
     * 用 `ctx.inject([...], cb)` 分叉是安全的 —— 服务不在时回调永不执行、不抛错。
     */
    function applySettings(ctx) {
      ensureSettingsStyle()
      try {
        // ⚠️ **两个 key 不是同一个字符串，绝不能混用**（真踩过，代价是卡片完全不可用）：
        //   * FORM_KEY  = **行 id** = `cordis.patch.yml` 里 `- id:` 的值 = `display-panel`；
        //     `configForms.get()`（≥0.1.7）与 `settingsScope.bind({namespace})`（≤0.1.6）
        //     都只认它。宿主半边 `SETTINGS_NAMESPACE` 必须与此一致。
        //   * SLOT_KEY  = **包名** = `dsh-display-panel`；卡片槽位
        //     `plugins.bundle.config` / `settings.plugin.item` 的 key 是它。
        // 混用的表现不是报错，而是"卡片在、里面空的、保存还说已保存"：
        // `configForms.get('<包名>')` 找不到 entry，返回一个 status:'unavailable' 的空表单。
        const FORM_KEY = 'display-panel'
        const SLOT_KEY = 'dsh-display-panel'
        const state = { scope: null, era: '' }
        let registered = false

        function acquire(scope, era) {
        ...
        // ① dsh ≥ 0.1.7：每个 live entry 一张 ConfigForm，key 是**行 id**。
        try {
          ctx.inject(['configForms'], (fctx) => {
            try {
              const forms = fctx && fctx.configForms
              if (!forms || typeof forms.get !== 'function') return
              const form = forms.get(FORM_KEY)
              // `get()` 对未知 id 也返回对象，所以这里**不能**靠它判"有没有设置面" ——
              // 真正的信号在快照里（status/writable），由卡片自己读（见 SettingsCard）。
              acquire(form, 'configForms')
            } catch (error) {
              console.warn('[dsh-display-panel] configForms 获取失败:', error)
            }
          })
        } catch (error) {
          console.warn('[dsh-display-panel] configForms 注入失败:', error)
        }

        // ② dsh ≤ 0.1.6：绑定宿主的 settings namespace（同一代的行 id 即 namespace）。
        try {
          ctx.inject(['settingsScope'], (sctx) => {
            try {
              const svc = sctx && sctx.settingsScope
              if (!svc || typeof svc.bind !== 'function') return
              acquire(svc.bind({ namespace: FORM_KEY }), 'settingsScope')
            } catch (error) {
              console.warn('[dsh-display-panel] settingsScope 获取失败:', error)
            }
          })
        } catch (error) {
          console.warn('[dsh-display-panel] settingsScope 注入失败:', error)
        }

        function registerCards() {
          if (registered) return
          registered = true
          const card = (props) => h(SettingsCard, Object.assign({}, props, { settingsScope: state.scope }))
          // ⚠️ 刻意**不声明** `locale:`。声明了它，渲染时会要求宿主装了 locale face
          // （`ui-slots` 的说明：缺失时 "fails loud"），而我们只需要一段固定文案 ——
          // 语言由本文件既有的 `pickLang()` 判（不引入新的必需服务）。
          try {
            // ③ 新槽位（0.1.6-alpha.2 起）：key = 包名。
            ctx.slots.inject('plugins.bundle.config', () =>
              ctx.slots.register(
                { name: 'plugins.bundle.config', key: SLOT_KEY },
                card,
              ),
            )
          } catch (error) {
            console.warn('[dsh-display-panel] plugins.bundle.config 注册失败:', error)
          }
          try {
            // ③ 旧槽位（≤ 0.1.6-alpha.1）：key = settings namespace。
            ctx.slots.inject('settings.plugin.item', () =>
              ctx.slots.register(
                { name: 'settings.plugin.item', key: FORM_KEY },
                card,
              ),
            )
          } catch (error) {
            console.warn('[dsh-display-panel] settings.plugin.item 注册失败:', error)
          }
        }
      } catch (error) {
        // 设置卡片出问题**绝不能**影响面板本身。
        console.warn('[dsh-display-panel] 设置卡片挂载失败:', error)
      }
    }
     ...
     * 当前界面语言，且**跟随切换**。
     *
     * `pickLang()` 是纯函数（读 `navigator.language`），换语言不会自己触发重渲染。
     * 这里用一个低频轮询兜底（2 秒一次、不读 DOM、不请求网络），换来"切语言后
     * 卡片文案跟着变"而**不必**依赖宿主某个版本才有的 locale 服务 —— 三代通吃。
     */
    function useLang() {
      const [lang, setLang] = React.useState(pickLang())
      React.useEffect(() => {
    ...
    /**
     * 分辨率写法校验：与宿主 schema（`SIZE_PATTERN` + `normalizeSize` 的 64..20000）一致。
     *
     * 前后端**必须同一套规则**：只在前端拦会让"宿主其实支持"的写法用不了
     * （例如 `1600` —— 宿主接受、会归一化成 1600x1600）；只在后端拦则会出现
     * "界面收下、宿主静默丢弃"。所以两边都判，且判同一件事。
     *
     * @param {string} text - 用户输入（已 trim）。
     * @returns {boolean} 是否合法。
     */
    function sizeOk(text) {
      const match = /^(\d{2,5})(?:\s*[xX*]\s*(\d{2,5}))?$/.exec(text)
      if (!match) return false
      const width = Number.parseInt(match[1], 10)
      if (!(width >= 64 && width <= 20000)) return false
      if (match[2] === undefined) return true
      const height = Number.parseInt(match[2], 10)
      return height >= 64 && height <= 20000
    }

    /**
     * 空闲回收校验：0..10080 的整数（与宿主 schema 的 min/max 一致）。
     *
     * 上界不是形式主义：`set` 被宿主拒绝时（超范围）卡片要**说出来**，
     * 而不是因为前端放行、后端拒绝，最后两边都不报错。
     *
     * @param {string} text - 用户输入（已 trim）。
     * @returns {boolean} 是否合法。
     */
    function idleOk(text) {
     ...
     * 一个字段的输入控件（受控 + 本地提交）。
     *
     * 为什么不直接受控于 scope 的快照：`ConfigForm.set` 是**异步**的
     * （要过宿主校验、落盘、回读），拿它当 onChange 会让输入框在中文字
     * 输入法组合期间被回写打断。所以这里维护本地草稿，点「保存」才提交。
     */
    function SettingsCard(props) {

      const scope = props && props.settingsScope ? props.settingsScope : null
      const lang = useLang()
      const t = TEXT[lang]
      ...
      React.useEffect(() => {
        ...
        try {
          ...
          console.warn('[dsh-display-panel] settings subscribe failed:', err)
          return undefined
        }
      }, [scope])
      ...
      // 快照的引用会在每次变更后换新，`version` 只是让重渲染发生（读的是上面这行）。
      void version

      /**
       * 这个设置面**到底能不能用**。
       *
       * ⚠️ 不能靠"scope 是不是 null"判（真踩过）：`configForms.get(id)` 与
       * `settingsScope.bind()` 对**未知的 key 也返回对象**，所以 `!scope` 永远是假，
       * "宿主没提供设置面"这条分支根本不可达。真正的信号在快照里：
       *   * `status === 'unavailable'`：宿主的 describe 里没有这个 namespace
       *     （行 id 不对、插件没被激活、或这一代压根没有设置面）；
       *   * `writable === false` 或 `mode === 'memory'`：只读（非回环页面等）。
       * 拿不到快照（scope 真的是 null）时同样按不可用处理。
       */
      const snapStatus = snapshot && typeof snapshot.status === 'string' ? snapshot.status : ''
      const readOnly = !!(snapshot && (snapshot.writable === false || snapshot.mode === 'memory'))
      const usable = !!scope && snapStatus !== 'unavailable' && !readOnly

      const text = (field) => {
        const raw = value[field]
        return raw === undefined || raw === null ? '' : String(raw)
      }
      const draftOf = (field) => (draft && Object.prototype.hasOwnProperty.call(draft, field)
        ? draft[field]
        : text(field))
      const setField = (field, next) => setDraft(Object.assign({}, draft, { [field]: next }))

      const sizeRaw = draftOf('size').trim()
      const idleRaw = draftOf('idleMinutes').trim()
      // 校验范围与宿主 schema **必须一致**（不一致的表现最坏：界面收下了、宿主静默丢弃，
      // 用户看到的值和生效值不是一回事）。宿主端：size 走 SIZE_PATTERN + normalizeSize
      // 的 64..20000；idleMinutes 是 0..10080 的整数。
      const sizeBad = sizeRaw !== '' && !sizeOk(sizeRaw)
      const idleBad = idleRaw !== '' && !idleOk(idleRaw)
      const inputOn = Boolean(value.inputEnabled)

      /**
       * 提交一项；空串走 unset（恢复"未设置"，由环境变量/默认值接手）。
       *
       * ⚠️ 返回值**必须检查**：0.1.7 的 `ConfigForm.set/unset` 返回
       * `Promise<boolean>`，`false` = 宿主拒绝或跳过（例如字段不 volatile、
       * revision 冲突）。早先无条件 await，被拒也提示"已保存" —— 用户看到的是
       * "改成功了"，而配置里什么都没变。
       *
       * @returns {Promise<boolean>} 是否被宿主接受（旧版 scope 不返回布尔 → 视为成功）。
       */
      const commit = async (field, raw) => {
        ...
        if (trimmed === '') {
          ...
          return answer === undefined ? true : answer !== false
        }
        let payload = trimmed
        if (field === 'size') {
          const parts = trimmed.split(/\s*[xX*]\s*/)
          payload = parts.length > 1 ? `${parts[0]}x${parts[1]}` : `${parts[0]}x${parts[0]}`
        } else if (field === 'idleMinutes') {
          payload = Number.parseInt(trimmed, 10)
        }
        const answer = await scope.set(field, payload)
        return answer === undefined ? true : answer !== false
      }
      ...
      const save = async () => {
        ...
        if (sizeBad || idleBad) {
          setError(sizeBad ? t.setInvalidSize : t.setInvalidIdle)
          return
        }
        setBusy(true)
        setError('')
        setNote('')
        try {
          const accepted = [await commit('size', sizeRaw), await commit('idleMinutes', idleRaw)]
          if (accepted.some((ok) => ok === false)) {
            setError(t.setRejected)
            return
          }
          setDraft(null)
          setNote(t.setSaved)
          // 这三项由**独立进程**消费，宿主要重启服务才生效：顺手催一次，
          // 失败也不算保存失败（值已经落在宿主配置里了）。
          void restartServiceQuietly()
        } catch (err) {
      ...
      const reset = async () => {
        ...
        try {
          let rejected = false
          for (const field of ['size', 'idleMinutes', 'inputEnabled']) {
            if (typeof scope.unset !== 'function') continue
            const answer = await scope.unset(field)
            if (answer === false) rejected = true
          }
          if (rejected) {
            setError(t.setRejected)
            return
      ...
      const toggleInput = async (next) => {
        if (!scope || typeof scope.set !== 'function' || busy) return
        setBusy(true)
        setError('')
        setNote('')
        try {
          const answer = await scope.set('inputEnabled', next)
          if (answer === false) {
            setError(t.setRejected)
            return
          }
      ...
      // 快照还没到手：先别下"不可用"的结论（`status: 'loading'` 是正常的第一瞬间）。
      if (scope && snapStatus === '' ) {
        return h('div', { className: 'ddp-set' },
          h('div', { className: 'ddp-set-head' },
            h('div', { className: 'ddp-set-title' }, t.setTitle),
            h('div', { className: 'ddp-set-intro' }, t.setLoading),
          ),
        )
      }
      // 真的没有可用的设置面：**如实说**（不再渲染一个空表单让用户以为是自己没填）。
      if (!usable) {
        return h('div', { className: 'ddp-set' },
          h('div', { className: 'ddp-set-head' },
            h('div', { className: 'ddp-set-title' }, t.setTitle),
            h('div', { className: 'ddp-set-intro' }, readOnly ? t.setReadOnly : t.setUnavailable),
          ),
        )
      }
      ...
      return h('div', { className: 'ddp-set' },
        h('div', { className: 'ddp-set-head' },
          h('div', { className: 'ddp-set-title' }, t.setTitle),
          h('div', { className: 'ddp-set-intro' }, t.setIntro),
        ),
        h('div', { className: 'ddp-set-field' },
          h('div', { className: 'ddp-set-label' }, t.setSize),
          h('input', {
            className: 'ddp-set-input', type: 'text', value: sizeRaw,
            placeholder: '1600x1000',
            'aria-label': t.setSize, 'aria-invalid': sizeBad ? 'true' : 'false',
            'data-field': 'size',
            onChange: (event) => setField('size', event.target.value),
          }),
          h('div', { className: 'ddp-set-hint' }, t.setSizeHint),
        ),
        h('div', { className: 'ddp-set-field' },
          h('div', { className: 'ddp-set-label' }, t.setIdle),
          h('input', {
            className: 'ddp-set-input', type: 'text', value: idleRaw,
            placeholder: '30',
            'aria-label': t.setIdle, 'aria-invalid': idleBad ? 'true' : 'false',
            'data-field': 'idleMinutes',
            onChange: (event) => setField('idleMinutes', event.target.value),
          }),
          h('div', { className: 'ddp-set-hint' }, t.setIdleHint),
        ),
        h('div', { className: 'ddp-set-field' },
          h('label', { className: 'ddp-set-toggle' },
            h('input', {
              type: 'checkbox', checked: inputOn, disabled: busy,
              'data-field': 'inputEnabled',
              onChange: (event) => { void toggleInput(event.target.checked) },
            }),
            h('span', null, t.setInput),
          ),
          h('div', { className: 'ddp-set-hint' }, t.setInputHint),
          inputOn ? h('div', { className: 'ddp-set-warn' }, t.setInputWarn) : null,
        ),
        h('div', { className: 'ddp-set-actions' },
          h('button', {
            type: 'button', className: 'ddp-btn ddp-btn-primary',
            disabled: busy || sizeBad || idleBad, onClick: () => { void save() },
          }, busy ? t.setSaving : t.setSave),
          h('button', {
            type: 'button', className: 'ddp-btn', disabled: busy,
            onClick: () => { void reset() },
          }, t.setReset),
          note ? h('span', { className: 'ddp-set-note' }, note) : null,
          error ? h('span', { className: 'ddp-set-err' }, error) : null,
        ),
      )
    }

    /** 改了设置后请宿主重启服务（失败只记日志：值已经存进宿主配置了）。 */
    async function restartServiceQuietly() {
      try {
        await fetch(API + '/service?action=restart', { method: 'POST' })
      } catch (error) {
        console.warn('[dsh-display-panel] 重启服务失败（设置已保存）:', error)
      }
    }

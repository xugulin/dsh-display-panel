  ...
  factory: (require) => {
    ...
    const TEXT = {
      zh: {
        ...
        idleClosedHint: '它会在第一次用到显示的时候自动打开（跑程序 / 注入输入 / 拉一帧），也可以现在打开。',
        idleEmptyTitle: '显示器空闲',
        idleEmptyHint: '这台显示上现在没有程序在运行。你看到的插画是面板自己画的，不占用显示器。',
        openDisplay: '打开显示器',
        closeDisplay: '关闭显示器',
      ...
      en: {
        ...
        idleClosedHint: 'It opens automatically the first time the display is used (run a program, inject input, grab a frame) — or open it now.',
        idleEmptyTitle: 'Display idle',
        idleEmptyHint: 'Nothing is running on this display right now. The artwork is drawn by the panel itself and uses no display resources.',
      ...
      const idleOverlay = (idleMode && quote)
        ? h('div', { className: 'ddp-idle', 'data-mode': idleMode, 'data-quote': String(quoteIdx),
          ...
          h('div', { className: 'ddp-idle-body' },
            ...
            h('div', { className: 'ddp-idle-sub' }, idleMode === 'closed' ? t.idleClosedHint : t.idleEmptyHint),

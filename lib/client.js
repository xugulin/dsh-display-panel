...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 宿主半边的接口：返回**本用户**的服务端口与访问令牌。
     *
     * 为什么要它：服务现在要求 `?k=<token>`（否则同机其他用户的面板会看到你的桌面）。
     * 令牌文件只有本用户读得到，而浏览器读不了文件 —— 由宿主（以该用户身份运行）
     * 读出来经这个接口转交。拿不到就退回"按端口探测、不带令牌"的老路。
     */
    const INFO_URL = '/api/dsh-display-panel/info'
    ...
    function Display(props) {
      ...
      // 服务地址与令牌：优先问宿主（本用户的 ✓），拿不到再按端口探测。
      const [origin, setOrigin] = useState('')
      const [token, setToken] = useState('')
      const base = (sid && origin) ? origin + '/s/' + encodeURIComponent(sid) + '/' : ''
      // 带令牌的地址（没有令牌时原样返回）
      const withK = (url) => (token ? url + (url.indexOf('?') < 0 ? '?' : '&') + 'k=' + token : url)
            ...
            if (res.ok) {
              ...
              if (info && info.port) {
                ...
                const probe = await fetch(
                  url + '/state?probe=1' + (info.token ? '&k=' + info.token : ''),
                  { cache: 'no-store' })
                if (probe.ok) {
                  if (alive) { setToken(info.token || ''); setOrigin(url) }
                  return
                }
                tok = info.token || ''
              }
            }
          } catch (error) { /* 宿主接口不可用（旧版宿主）→ 走下面的老路 */ }
          // ② 兜底：按端口顺序探测，取第一个应答的（旧行为，不带令牌）
          for (const port of PORTS) {
            const url = 'http://127.0.0.1:' + port
            try {
              const res = await fetch(url + '/state?probe=1' + (tok ? '&k=' + tok : ''),
                                      { cache: 'no-store' })
              if (res.ok) { if (alive) { setToken(tok); setOrigin(url) }; return }
      ...
      useEffect(() => {
        ...
        const probe = async () => {
          try {
            const res = await fetch(withK(base + cacheBust), { cache: 'no-store' })

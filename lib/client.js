...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    const PROFILES = {
      auto: { label: 'auto', fps: 15, quality: 70, scale: 1 },
      smooth: { label: 'smooth', fps: 24, quality: 62, scale: 1 },
      saver: { label: 'saver', fps: 8, quality: 55, scale: 0.75 },
      paused: { label: 'paused', fps: 2, quality: 50, scale: 0.5 },
    }
    ...
    function Display(props) {
        ...
        const sendStreamConfig = async (name, reason) => {
          ...
          try {
            const res = await fetch(API + STREAM_CONFIG_PATH + '?session=' + encodeURIComponent(sid), {
              ...
              cache: 'no-store',
              signal: ctrl.signal,
              body: JSON.stringify({ profile: name, fps: cfg.fps, quality: cfg.quality, scale: cfg.scale, reason: reason || '' }),
            })
            if (res && res.ok) {
              const json = await readJsonSafe(res)
      ...
      if (phase === 'streaming') {
        badges.push(h('span', { key: 'fps', className: 'ddp-badge ddp-ok' }, (stats.fps || 0).toFixed(1) + ' ' + t.fps))
        if (stats.dropped > 0) {

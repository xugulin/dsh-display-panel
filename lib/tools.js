...
function defineTools({ discover, ensureService, logger = console }) {
  ...
  async function callService({ session = '', path, method = 'GET', body, raw = false, ensure = true, timeoutMs = TIMEOUT_MS }) {
    ...
    const query = state.token ? `?k=${encodeURIComponent(state.token)}` : ''
      ...
      if (session && service.running === true) {
        try {
          ...
          out.hint = state.realDesktop === true
            ...
            : `在会话显示上跑程序请用 ${PREFIX}run（不要用 DISPLAY=:N —— 沙箱里看不到那个 socket）。`
        } catch (error) {
  ...
  const run = {
    ...
    description:
      ...
      + 'and output). Use this instead of `DISPLAY=:N cmd`: the DSH shell tool runs in a sandbox whose /tmp is a '
      + 'private tmpfs, so the X socket the viewer created is invisible there — this tool starts the program from '
      + 'the viewer process itself, which can see it. The command inherits DISPLAY (and QT_QPA_PLATFORM=xcb on Linux).',
    parameters: {
      type: 'object',
      properties: {
  ...
  const screenshot = {
    ...
    description:
      ...
      + 'picture (read the file with the image reader) or hand the path to a person.',
    parameters: {
      type: 'object',
      properties: {
    ...
    execute: async (args, exec) => {
      const session = sessionOf(exec)
      const { buffer } = await callService({ session, path: '/snapshot', raw: true, timeoutMs: 8000 })
      ...
      return clean({ path: target, bytes: buffer.length, jpeg: buffer[0] === 0xFF && buffer[1] === 0xD8 })
    },
  }
  ...
  if (logger && typeof logger.log === 'function') {
    logger.log(`[dsh-display-panel] tools: ${[status, open, sessions, run, procs, screenshot, input].map((t) => t.name).join(', ')}`)
  }
  return [status, open, sessions, run, procs, screenshot, input]
}
